...
void threadWork( void *node)	
{
	...
	while(active){
		...
		//pthread_cleanup_push(pthread_mutex_unlock, (void *) &(tmp_control.mutex));	//防止死锁	111111  具体处理函数可自定义 参数也一样
...
//			printf("---	%d	thread actived here\n",pthread_self());
		...
		threadnode->task = (void*)NULL;				//置空该任务
		threadnode->busy = 0;							//这样该线程执行完成以后就是非忙状态了
		pthread_mutex_unlock(&(threadnode->control.mutex));
		pthread_mutex_lock(&(threadpool->mutex));
		threadpool->t_idleNum++;
		pthread_mutex_unlock(&(threadpool->mutex));
		//pthread_cleanup_pop(0);					//11111
		pthread_setcanceltype(oldtype, NULL);		//取出原来的 对cancel的反应状态
...
static void* managerThreadNum(void* thrdpool)
{
	...
	while(1){ 
		...
		//连续3次发现空闲线程太多，就销毁一定数量的空闲线程  过少就创建
...
//void managerDestroy(pthread_t mng_inputTsk, pthread_t mng_task2work, pthread_t mng_thrNumCtrl)
//{
//	//int worknum=0;			//垃圾
//	//do{											//直到所有work 都完成后
//	//        pthread_mutex_lock(&(threadpool->mutex));
